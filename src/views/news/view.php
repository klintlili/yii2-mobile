...
<div class="wrap news_view">
    <div class="snor_w clearfix">
        <div class="view_left">
            <div class="snor_path"><a href="<?=Url::to(['/news/index']);?>">新闻中心</a>&gt;<a href="<?=Url::to(['/news/index', 'code' => $model->code]);?>"><span><?=$model->cateName;?></span></a></div>
            ...
            <div class="pre_next_box">
                ...
                <?php if (!empty($preArticle)) { ?>
                    <a href="<?=Url::to(['/news/view', 'id' => $preArticle->id])?>" class="ellipse1" title="<?=$preArticle->title;?>">上一新闻：<?=$preArticle->title;?></a>
                <?php } ?>
                <?php $nextArticle = $model->nextArticle; ?>
                <?php /* @var $nextArticle snor\web\models\News */ ?>
                <?php if (!empty($nextArticle)) { ?>
                    <a href="<?=Url::to(['/news/view', 'id' => $nextArticle->id])?>" class="ellipse1" title="<?=$nextArticle->title;?>">下一新闻：<?=$nextArticle->title;?></a>
                <?php } ?>
                ...
                <div class="project_about_box">
                    ...
                    <ul class="clearfix">
                        ...
                        <?php foreach ($relatedArticles as $relatedArticle){?>
                            <li><a href="<?=Url::to(['/news/view', 'id' => $relatedArticle->id])?>" class="ellipse1"><?=$relatedArticle->title;?></a></li>
                        <?php } ?>
                    </ul>
                </div>
            <?php } ?>
            ...
            <div class="view_right">
                ...
                <dl  class="new_projects">
                    ...
                    <?php foreach ($topNewArticles as $topNewArticle){ ?>
                        <dd>
                            <a href="<?=Url::to(['/news/view', 'id' => $topNewArticle->id]);?>" class="block_a">
                            ...
                            <div class="new_project_bot clearfix">
                                ...
                                <span class="more" href="<?=Url::to(['/news/view', 'id' => $topNewArticle->id]);?>">更多 &gt;</span>
                            </div>
                            </a>
                        </dd>
                    <?php } ?>
